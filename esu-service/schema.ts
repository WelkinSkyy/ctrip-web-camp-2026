/**
 * 酒店管理系统 - Drizzle ORM 数据库模式定义
 *
 * 本文件定义了酒店预订平台的所有数据库表结构和关系映射。
 * 使用 Drizzle ORM (最新beta版) 的 PostgreSQL 适配器。
 *
 * 数据模型包括：
 * - 用户系统 (users)：支持多角色（customer/merchant/admin）
 * - 酒店管理 (hotels)：包含酒店基本信息和状态
 * - 房型管理 (roomTypes)：酒店下的各类房型
 * - 优惠活动 (promotions)：关联酒店或房型的优惠信息
 * - 房型优惠关联 (roomTypePromotion)：多对多关系中间表
 * - 预订系统 (bookings)：用户预订记录
 */
