...
export const ParamIdSchema = v.pipe(
  v.string(),
  v.toNumber(),
  v.integer(),
);
...
export const UserSchema = v.object({
  ...
  username: v.pipe(
    v.string(),
    v.minLength(3, '用户名至少3字符'),
    v.maxLength(50, '用户名最多50字符'),
  ), // 用户名
  password: v.pipe(
    v.string(),
  ...
  phone: v.nullable(
    v.pipe(v.string(), v.minLength(6, '手机号至少6位')),
  ), // 手机号，可选
  email: v.nullable(
    v.pipe(v.string(), v.email('无效邮箱')),
  ), // 邮箱，可选
  ...vTimestamps(),
});
...
export const UserResponseSchema = v.omit(UserSchema, [
  'password',
]);

// 用户注册Request Schema（无ID、时间，password明文）
export const UserRegisterRequestSchema = v.omit(
  UserSchema,
  ['id', 'createdAt', 'updatedAt', 'deletedAt'],
);

export const UserRegisterResponseSchema = v.omit(
  UserSchema,
  ['password'],
);

export const UserLoginRequestSchema = v.pick(UserSchema, [
  'username',
  'password',
]);
...
export const HotelSchema = v.object({
  ...
  nearbyAttractions: v.optional(
    v.array(
      v.pipe(
        v.string(),
        v.minLength(1, '最少输入一个字符'),
        v.maxLength(50, '最多输入50个字符'),
      ),
    ),
  ), // 附近景点
  images: v.optional(
    v.array(v.pipe(v.string(), v.url('无效URL'))),
  ), // 图片
...
export const HotelCreateRequestSchema = v.omit(
  HotelSchema,
...
// 房型创建/更新 Partial
export const PartialRoomTypeSchema =
  v.partial(RoomTypeSchema);
...
// 优惠创建/更新 Partial
export const PartialPromotionSchema =
  v.partial(PromotionSchema);
...
export const BookingCreateSchema = v.omit(BookingSchema, [
  'id',
  'createdAt',
  'updatedAt',
  'deletedAt',
...
export const hotelsContract = c.router({
  ...
  update: {
    ...
    summary:
      '编辑酒店（商户仅自己的，pending/rejected可编，admin无限）',
    metadata: { permission: ['merchant', 'admin'] },
  },
  approve: {
    method: 'POST',
...
export const bookingsContract = c.router({
  ...
  cancel: {
    ...
    summary:
      '取消预订（用户/商户/admin，恢复库存，status → cancelled）',
    metadata: {
      permission: ['customer', 'merchant', 'admin'],
    },
  },
